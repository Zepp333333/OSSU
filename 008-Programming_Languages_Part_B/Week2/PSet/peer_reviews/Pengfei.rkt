        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if (and (int? v1)
               ...
               (error "MUPL ifgreater applied to non-number: v1 = " v1 ", v2 = " v2)))]
        [(mlet? e)
         (let ([v (mlet-var e)])
         ...
         (let* ([c (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? c)
               (eval-under-env (fun-body (closure-fun c))
                               (if (false? (fun-nameopt (closure-fun c)))
                                   (cons (cons (fun-formal (closure-fun c)) v)
                                         (closure-env c))
                                   (cons (cons (fun-nameopt (closure-fun c)) c)
                                         (cons (cons (fun-formal (closure-fun c)) v)
                                               (closure-env c)))))
               (error "MUPL call applied to non-closure:" c)))]
...
(define (mlet* lstlst e2)
  (if (null? lstlst)
      e2
      (let ([current-v (car lstlst)])
        (mlet (car current-v) (cdr current-v) (mlet* (cdr lstlst) e2)))))

;; (c)
(define (ifeq e1 e2 e3 e4)
  (ifaunit (ifgreater e1 e2 (int 1) (aunit))
           (ifaunit (ifgreater e2 e1 (int 1) (aunit))
                    e3
                    e4)
           e4))
       ...
       (fun #f "ls" 
            (ifaunit (var "ls")
                     ...
                     (apair (call (var "f") (fst (var "ls")))
                            (call (call (var "mmap") (var "f")) (snd (var "ls"))))))))
