...
class ResistantVirus(SimpleVirus):
	...
	def reproduce(self, popDensity, activeDrugs):
		...
		if len(activeDrugs) == 0:
			...
			# check if drugResist is a subset of true resistance.
			if set(activeDrugs).issubset(true_resistances):
				resistant = True
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
	...
	for patient in patients:
		...
		for step in range(TIMESTEPS):
			total_pop.append(patient.getTotalPop())
			resist_pop.append(getResistPop(resistances))
			patient.update()
		out_pop.append(total_pop)
		out_resist.append((resist_pop))
