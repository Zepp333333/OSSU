...
def generate_models(x, y, degs):
    ...
    for deg in degs:
        ...
        # models.append(np.array(np.polyfit(x, y, deg)))
    return(models)

### Test code for generate_models
...
def r_squared(y, estimated):
    ...
    SEE = ((np.array(y) - np.array(estimated))**2).sum()
    MV = ((np.array(y) - mean)**2).sum()
    return 1 - SEE / MV

# test code for r_squared
# y = [1, 2, 3, 4, 5]
# estimated = [2, 3, 3, 4, 5]
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        r2 = r_squared(yVals, estYVals)
        plt.figure()
        plt.title('Temp over time' + '\n' + 'model fit for ' + str(degree - 1)  +
                  ' degree(s) and' + '\n' + 'r2 = ' + str(round(r2, 3)))
        plt.xlabel('Year')
        plt.ylabel('Temp (C)')
        plt.plot(xVals, yVals, 'bo', label='Measured temp')
        plt.plot(xVals, estYVals, 'r',
                 label='model fit, degree = ' + str(degree - 1))
        plt.legend(loc='best')
        plt.tight_layout()
    plt.show()
...
# y = {}
# z = []
# x = INTERVAL_1 + INTERVAL_2
# for year in x:
#     y[year] = raw_data.get_yearly_temp('BOSTON', year)
# for year in y:
#     z.append(y[year].sum()/float(len(y[year])))
# print(z)
# models = generate_models(x, z, [1, 2, 4, 8, 16])
# evaluate_models_on_training(x, z, models)
