...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname space-invaders-starter4) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;;  1) calling next-xxx (where xxx is invaders, missiles tank)
;;  2) wrapping int next inviders and missiles the check-hit-x call to clean-up
;; missiles and invader that hit each-other.
...
(check-random (new-invader LOI0) (cons (make-invader (random WIDTH) 0 12) LOI0))
...
(check-expect (next-missiles LOM1) (cons
                                    ...
                                    empty))
(check-expect (next-missiles (cons (make-missile 200 0) LOM1))
              (cons
               (make-missile (missile-x M1)
                             (- (missile-y M1) MISSILE-SPEED))
               empty))
...
;---------------------------------------------------------------------------------------
;; ; Game -> Boolean
;; produce true if game is lost (invider landed)
