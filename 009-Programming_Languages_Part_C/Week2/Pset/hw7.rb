...
class GeometryValue 
  ...
  # moved from subclasses to parent
  def preprocess_prog
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog 
    if real_close_point(x1,y1,x2,y2)
      ...
      if (real_close(x1, x2) and (y2 < y1)) or (x2 < x1)
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1, @x2) #segments are on a vertical line
      ...
      (bXstart,bYstart,bXend,bYend)) =  @y1 < seg.y1 ? \
      [self.get_coords, seg.get_coords] : [seg.get_coords, self.get_coords]
      
      if real_close(aYend, bYstart)
        Pint(aXend, aYend) # just touching
      elsif aYend < bYstart
      ...
      ((aXstart,aYstart,aXend,aYend), \
      (bXstart,bYstart,bXend,bYend)) =  @x1 < seg.x1 ? \
      [self.get_coords, seg.get_coords] : [seg.get_coords, self.get_coords]
